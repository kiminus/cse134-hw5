...
const deviceBatteryLevel = document.getElementById('device-battery-level');
const devices = [
    {
        deviceID: 1,
        deviceName: 'Device 1',
...
function createDevice(deviceID, deviceName, deviceType, deviceStatus, deviceBatteryLevel) {
...
function updateDevice(deviceID, deviceName, deviceType, deviceStatus, deviceBatteryLevel) {
    const loaded_device = localStorage.getItem(deviceID.toString());
    if (loaded_device) {
        ...
        if (deviceStatus)
            updatedDevice.deviceStatus = deviceStatus;
        if (!isNaN(deviceBatteryLevel))
            updatedDevice.deviceBatteryLevel = deviceBatteryLevel;
