...
#page-link:hover {
    background: #573b7a;
        ...
        `;
        const avatar = this.shadowRoot!.getElementById('avatar') as HTMLElement;
        const title = this.shadowRoot!.querySelector('.title') as HTMLHeadingElement;
        const subtitle = this.shadowRoot!.querySelector('.subtitle') as HTMLHeadingElement;
        const menuIcon = this.shadowRoot!.getElementById('menu-icon') as HTMLElement;
        const image = this.shadowRoot!.querySelector('.image-container img') as HTMLImageElement;
        const articleDesc = this.shadowRoot!.getElementById('article-desc') as HTMLImageElement;
        const pageLink = this.shadowRoot!.getElementById('page-link') as HTMLAnchorElement;
    ...
    getBackgroundColor(fullName: string): string {
        if (!fullName) return "#0000FF"; // Equivalent to Color.Blue

        try {
            // Extract uppercase letters using regex
            const result = fullName.match(/[A-Z]/g)?.join("") || "";
            let hash = 1;
            if (result.length > 1) {
